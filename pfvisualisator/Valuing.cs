...
#region--------------------------Свойства объекта-----------------------------------------
    ...
    public class ValuWorka {
        ...
        public void AddValuSet(List<string> seta, vlEngino ten, int dlito, int kvo) {
            ...
            if (kvo != mano.Count) {
                throw new VisualisatorException(string.Format("Valuing->AddValuSet, kvo != mano.Count --{0}--{1}--", kvo, mano.Count));
                }
        ...
        private static string AnalizeVanStroke(string xx, out int pvalo, out int pdep, out long pnod, out int prng) {
            ...
            if (!int.TryParse(rr, out prng)) {
                throw new VisualisatorException(string.Format("Valuing->AalizeVanStroke, не определил ранг оценки --{0}--", rr));
                }
            patera = @" depth (\d+)";
            resa = Regex.Match(xx, patera);
            rr = resa.Groups[1].Value;
            if (!int.TryParse(rr, out pdep)) {
                throw new VisualisatorException(string.Format("Valuing->AalizeVanStroke, не определил глубину --{0}--", rr));
                }
            patera = @"score cp (-?\d+)";
            resa = Regex.Match(xx, patera);
            rr = resa.Groups[1].Value;
            if (!int.TryParse(rr, out pvalo)) {
                throw new VisualisatorException(string.Format("Valuing->AalizeVanStroke, не определил цифровую оценку --{0}--", rr));
                }
            patera = @"nodes (\d+)";
            resa = Regex.Match(xx, patera);
            rr = resa.Groups[1].Value;
            if (!long.TryParse(rr, out pnod)) {
                throw new VisualisatorException(string.Format("Valuing->AalizeVanStroke, не определил узелки --{0}--", rr));
                }

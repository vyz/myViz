...
#region------------------------------Статические массивы с возможными наборами полей-----------------------------------------
        ...
        private Mova Decode(string parst) {
            ...
            if (pst == "O-O") {
                ...
                if (pst.Length > 2) {
                    throw new myClasterException(string.Format("PozoUtils-Decode, pst.Length > 2 --{0}--", parst));
                    }
                ...
                if (lifi.Count == 0) { //Невозможное случается - Конкретно для автодосок Король на d5 и под шах :(
                    ...
                    if (Neodno) {
                        if (Neodnostr[0] >= 'a' && Neodnostr[0] <= 'h') {
                            ...
                            throw new myClasterException("PozoUtils-Decode, неправильный формат указания нального поля хода");
                            }
                    } else {
                        throw new myClasterException(string.Format("PozoUtils-Decode, неопределенность начального поля может не одна, но указаний нет --{0}--", parst));
                        }
                } else {
                    pf = lifi[0];
                    }
            ...
            if( Zapret ) {
                throw new myClasterException("PozoUtils, нарисовался ЗАПРЕТ");
                }
        ...
        private Mova DecodeFromTo(string fromto) {
            ...
            if (Zapret) {
                ...
                throw new myClasterException(erre);
                }
